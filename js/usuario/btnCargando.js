...
const btnGenerar = document.querySelector(".usuario__BtnGenerarLicencia");
/*=============================================
=                MARK:Botones                 =
=============================================*/

const htmlCerrarSesion = `
  Cerrar Sesión &nbsp;
  <svg
...
const htmlBoton = `
  ...
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="loading">
    <path
...
animacionCarga = (tipo) => {
  const contenidoHTML = htmlBoton.replace(
    "{{titulo}}",
    tipo === "cerrarSesion" ? "Cerrando Sesión" : "Generando"
  );

  if (tipo === "cerrarSesion") {
    btnCerrarSesion.innerHTML = contenidoHTML;
  } else {
    btnGenerar.innerHTML = contenidoHTML;
  }
};

quitarAnimacionCarga = (tipo) => {
  if (tipo === "cerrarSesion") {
    btnCerrarSesion.innerHTML = htmlCerrarSesion;
  } else {
    btnGenerar.innerHTML = htmlGenerar;
  }
};

/*============  End of Animacion Carga  =============*/

btnCerrarSesion.addEventListener("click", () => animacionCarga("cerrarSesion"));

btnGenerar.addEventListener("click", () => {
  obtenerInputRadioSeleccionado();
  
  if (valorSeleccionado !== undefined) {
    animacionCarga("generar");
  }
});
